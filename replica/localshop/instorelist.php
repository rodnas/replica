...
if ($key_m <> "")
	{
	$strmassql = "SELECT " . $modul_select . "_topic.*,partner.name AS partnername, store_topic.name AS storename";
	$strmassql .= " FROM " . $modul_select . "_topic";
	$strmassql .= " LEFT JOIN partner ON partner.id=".$modul_select."_topic.partner_id";
	$strmassql .= " LEFT JOIN store_topic ON store_topic.id=".$modul_select."_topic.store_id";
	$strmassql .= " WHERE ";	
...
$strsql .= " product_maker.name AS makername, product_topic.name AS typename, product_topic.tree_id AS tree_id, product_size.name AS sizename";
...
$strsql .= " LEFT JOIN product_topic ON product.topic_id=product_type.id";
...
if ($_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 2 ||
	...
	{
	$treecountSQL .= " product.active=1 AND product_type.tree_id LIKE ";
	}
$Page = $x_tree_page;
$treeAdmin = false;
...
if ($key_m <> "")
	...
	$strmassql .= "partner.name AS partnername,";
	$strmassql .= " store_topic.name AS storename, ".$modul_select."_type.name AS typename";
	$strmassql .= " FROM " . $modul_select . "_topic";
	$strmassql .= " LEFT JOIN ".$modul_select."_type ON ".$modul_select."_type.id=".$modul_select."_topic.type_id";
	$strmassql .= " LEFT JOIN partner ON partner.id=".$modul_select."_topic.partner_id";
	$strmassql .= " LEFT JOIN store_topic ON store_topic.id=".$modul_select."_topic.store_id";
	$strmassql .= " WHERE ";	
	$strmassql .= "(".$modul_select."_topic.id = " . $key_m  . ")";	
...
if ($key_m <> "")
	...
	if (mysqli_num_rows($rsMas) > 0)
		...
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle($modul_select."_topic","type_id")."&nbsp;</span></td>";
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle($modul_select."_topic","store_id")."&nbsp;</span></td>";
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle($modul_select."_topic","partner_id")."&nbsp;</span></td>";
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle($modul_select."_topic","delivery_letter")."&nbsp;</span></td>";
		$html_page .= "<td width='80' align='right'><span class='phpmaker'>&nbsp;".viewTitle($modul_select."_topic","topic_sum")."&nbsp;</span></td>";
		$html_page .= "<td width='90' align='right'><span class='phpmaker'>&nbsp;".viewTitle($modul_select."_topic","topic_inprice")."&nbsp;</span></td>";
		$html_page .= "<td width='90' align='right'><span class='phpmaker'>&nbsp;".viewTitle($modul_select."_topic","topic_outprice")."&nbsp;</span></td>";
...
$html_page .= "<td width='100' align='center'><span class='phpmaker'>&nbsp;".viewTitle($modul_select,"inprice")."&nbsp;</span></td>";
$html_page .= "<td width='100' align='center'><span class='phpmaker'>&nbsp;".viewTitle($modul_select,"outprice")."&nbsp;</span></td>";
...
*/
		...
		$storeSQL .= " WHERE store.topic_id=".$x_store_id." AND store.product_id=".$x_product_id;
		$storers = mysqli_query($GLOBALS['conn'],$storeSQL);
		if ($storers && $rowwrk = mysqli_fetch_array($storers))
			{
			$x_amount = $rowwrk["amount"];
			$x_inprice = $rowwrk["inprice"];
			$x_outprice = $rowwrk["outprice"];
			}
		else
			{
			$x_amount = "";
		...
		if ($x_topic_active)
			...
			$html_page .= "<td align='right'><span class='phpmaker'>&nbsp;&nbsp;" . $x_inprice ."&nbsp;</span></td>";
			$html_page .= "</td>";
			}
		$html_page .= activeTd($x_active,$enable["yes"],$enable["no"]);
		$basketsql = "SELECT * FROM ".$modul_select;
		...
		if ($x_topic_active)
			...
			if ($basket_is == 0)
				...
				$html_page .= "<input type='text' name='b_inprice' size='9' value=''  style='text-align:right;'>&nbsp;";
				$html_page .= "</td><td align='center'>";
				$html_page .= "<input type='text' name='b_outprice' size='9' value=''  style='text-align:right;'>&nbsp;";
				} 
			else 
				{
				$html_page .= "<input type='text' name='b_amount' size='3' value='" . $x_basket_amount . "' style='text-align:right;color:red;'>&nbsp;";
				$html_page .= "</td><td align='center'>";
				$html_page .= "<input type='text' name='b_inprice' size='9' value='" . $x_basket_inprice . "' style='text-align:right;color:red;'>&nbsp;";
				$html_page .= "</td><td align='center'>";
			...
			$html_page .= "<span class='phpmaker'>".$x_basket_inprice."&nbsp;</span>";
			$html_page .= "</td><td align='right'>";
			$html_page .= "<span class='phpmaker'>".$x_basket_outprice."&nbsp;</span>";
			}
		$html_page .= "</td>";
		$x_total_sum = $x_basket_amount*$x_basket_price;
...
function move(t,x,y,ip,op)
	...
	window.location = "index.php?modul_action=add&topic_id=" + t + "&key=" + x + "&amount=" + y + "&inprice=" + ip + "&outprice=" + op;
	}
</script>
