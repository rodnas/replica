...
$sqlKey = viewModul($modul_select,"name").".id=" . "" . $key . "";
// get action
$a = @$_POST["a"];
if (empty($a))
	{
...
switch ($a)
	...
	case "I": // display
		...
		break;
	case "D": // delete
		$strsql = "DELETE FROM " . viewModul($modul_select,"name") . " WHERE " . $sqlKey;
		$rs =	mysqli_query($GLOBALS['conn'],$strsql) or die(mysqli_error());
		$strsql = "DELETE FROM " . viewModul($modul_select,"itemtable") . " WHERE topic_id=" . $key;
		$rs =	mysqli_query($GLOBALS['conn'],$strsql) or die(mysqli_error());
		jumptopage("index.php?modul_action=" . $which_back);
		break;
	}
...
//		$bgcolor=$color5; // alternate row color
	...
	$html_page .= "<td bgcolor='" . $color6 . "'><span class='phpmaker'>&nbsp;".$x_storename;
	$html_page .= "&nbsp;</span></td>";
	$html_page .= "</tr>";
	$html_page .= "</table>";
	$html_page .= "</td>";
