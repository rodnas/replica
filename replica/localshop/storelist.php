...
if ($x_category == "basket")
	...
	$_SESSION[$which_system.$modul_select . "_REC"] = $startRec;
	$searchwhere .= "product.id=".$modul_select.".product_id ";
...
if ($x_category == "minimax")
	...
	$startRec = 1; //reset start record counter (reset command)
	$_SESSION[$which_system.$modul_select . "_REC"] = $startRec;
	$searchwhere .= "product.description LIKE '%minimax%'";
	$_SESSION[$which_system.$modul_select . "_searchwhere"] = $searchwhere;
	}
$x_Page = @$_GET["Page"];
if (isset($x_Page))
...
if ($_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 2 ||
	...
	{
	$DefaultFilter = "product_type.tree_id LIKE '%'";
	}
else
	{
	$DefaultFilter = $modul_select.".active=1 AND product_type.tree_id LIKE '%'";
	}
$DefaultFilter .= " AND product.id=".$modul_select.".product_id ";
$DefaultFilter .= " AND ".$modul_select.".topic_id=".$key_m;
checkorder();
// build SQL
$strsql = "SELECT product.*,".$modul_select.".amount, ".$modul_select.".inprice, ".$modul_select.".outprice, ";
...
$noveltySQL = "SELECT COUNT(*) AS noveltycount";
$noveltySQL .= " FROM product, ".$modul_select." ";
$noveltySQL .= " WHERE product.id=".$modul_select.".product_id ";
$noveltySQL .= " AND ".$modul_select.".topic_id=".$key_m ." AND product.novelty=1";
...
$html_page .= "</a>";
$html_page .= "</td></tr>";
$saleSQL = "SELECT COUNT(*) AS salecount";
$saleSQL .= " FROM product, ".$modul_select." ";
$saleSQL .= " WHERE product.id=".$modul_select.".product_id ";
$saleSQL .= " AND ".$modul_select.".topic_id=".$key_m ." AND product.sale=1";
...
$html_page .= "</a>";
$html_page .= "</td></tr>";
$minimaxSQL = "SELECT COUNT(*) AS minimaxcount";
$minimaxSQL .= " FROM product, ".$modul_select." ";
$minimaxSQL .= " WHERE product.id=".$modul_select.".product_id ";
$minimaxSQL .= " AND ".$modul_select.".topic_id=".$key_m ." AND";
$minimaxSQL .= " product.description LIKE '%minimax%'";
...
$treecountSQL .= " FROM product, ".$modul_select." ";
$treecountSQL .= " LEFT JOIN product_type ON product.type_id=product_type.id";
$treecountSQL .= " WHERE product.id=".$modul_select.".product_id ";
$treecountSQL .= " AND ".$modul_select.".topic_id=".$key_m ." AND ";
if ($_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 2 ||
	$_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 3)
	{
	$treecountSQL .= " product_type.tree_id LIKE " ;
	}
else
	{
...
if ($key_m <> "")
	...
	if (mysqli_num_rows($rsMas) > 0)
		...
		$x_topic_iprice = $rowwrk["topic_inprice"];
...
*/
		...
		$html_page .= "<span class='phpmaker'>" . $x_amount . "&nbsp;</span>";
		$html_page .= "</td><td align='center'>";
		$x_total_inprice=$x_amount*$x_inprice;
		if ($x_total_inprice==0) $x_total_inprice = "";
		$x_total_outprice=$x_amount*$x_outprice;
		if ($x_total_outprice==0) $x_total_outprice = "";
		$html_page .= "<span class='phpmaker'>" . $x_inprice . " / ".$x_total_inprice."&nbsp;</span>";
		$html_page .= "</td><td align='center'>";
		$html_page .= "<span class='phpmaker'>" . $x_outprice . " / ".$x_total_outprice. "&nbsp;</span>";
		$html_page .= "</td>";
