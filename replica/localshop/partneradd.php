...
switch ($a)
	...
	case "A": // add
		...
		sharefrompost();
		// Duplicate controll
		$strsql = "SELECT * FROM " . $modul_select . " WHERE name='" . $x_name . "' AND country='" . $x_country . "' AND zipcode='" . $x_zipcode . "' AND city='" . $x_city . "' AND address='" . $x_address . "'";
		...
		if ($x_errortext == 'NULL' || $x_errortext == "")		
			...
			if (@$HTTP_POST_FILES['x_pictureorig']['name']!="")
				...
				|| ($imagetype=="image/gif") || ($imagetype=="image/png"))
					...
					$pictnameorig = realpath($modul_image_path) . "/partner_" . $last_id . "_orig" . $extension;
					$pictnamesmall = realpath($modul_image_path) . "/partner_" . $last_id . "_small" . $extension;
					$pictnameSAVE = "partner_" . $last_id . "_small" . $extension;
