...
if ($key_m <> "")
	{
//	$masterdetailwhere = "store.topic_id = " . $key_m  . "";
//	$masterdetailwhere = $modul_select."_topic.id = " . $key_m  . "";
	$strmassql = "SELECT " . viewModul($modul_select,"name") . "_topic.*,partner.name AS partnername, store_topic.name AS storename";
	$strmassql .= " FROM " . viewModul($modul_select,"name") . "_topic";
	$strmassql .= " LEFT JOIN partner ON partner.id=".viewModul($modul_select,"name")."_topic.partner_id";
	$strmassql .= " LEFT JOIN store_topic ON store_topic.id=".viewModul($modul_select,"name")."_topic.store_id";
	$strmassql .= " WHERE ";	
	$strmassql .= "(".viewModul($modul_select,"name")."_topic.id = " . $key_m  . ")";	
...
if ($pSearch <> "")
	...
	if ($pSearchType <> "")
		...
		foreach ($arpSearch as $kw)
			{
			$b_search .= "(";
			$b_search .= "product.productid LIKE '%" . trim($kw) . "%' OR ";
			$b_search .= "product.barcode LIKE '%" . trim($kw) . "%' OR ";
			$b_search .= "product.name LIKE '%" . trim($kw) . "%' OR ";
			$b_search .= "product_maker.name LIKE '%" . trim($kw) . "%' OR ";
			$b_search .= "product_type.name LIKE '%" . trim($kw) . "%' OR ";
			$b_search .= "product_size.name LIKE '%" . trim($kw) . "%' OR ";
			$b_search .= "product.description LIKE '%" . trim($kw) . "%' OR ";
		...
		{
		$b_search .= "product.productid LIKE '%" . $pSearch . "%' OR ";
		$b_search .= "product.barcode LIKE '%" . $pSearch . "%' OR ";
		$b_search .= "product.name LIKE '%" . $pSearch . "%' OR ";
		$b_search .= "product_maker.name LIKE '%" . $pSearch . "%' OR ";
		$b_search .= "product_type.name LIKE '%" . $pSearch . "%' OR ";
		$b_search .= "product_size.name LIKE '%" . $pSearch . "%' OR ";
		$b_search .= "product.description LIKE '%" . $pSearch . "%' OR ";
		}
	}
if (substr($b_search, -4) == " OR ")
	{
...
if ($x_category == "basket")
	...
	$_SESSION[$which_system.$modul_select . "_REC"] = $startRec;
	$searchwhere .= "store.product_id=".viewModul($modul_select,"name").".product_id ";
	$searchwhere .= " AND ".viewModul($modul_select,"name").".topic_id=".$key_m;
	$_SESSION[$which_system.$modul_select . "_searchwhere"] = $searchwhere;
	}
if ($x_category == "minimax")
	...
	$startRec = 1; //reset start record counter (reset command)
	$_SESSION[$which_system.$modul_select . "_REC"] = $startRec;
	$searchwhere .= viewModul($modul_select,"name") . ".description LIKE '%minimax%'";
	$_SESSION[$which_system.$modul_select . "_searchwhere"] = $searchwhere;
	}
$x_Page = @$_GET["Page"];
if (isset($x_Page))
	...
	$treeSQL = "SELECT * FROM product_type WHERE id=".$x_Page;
	...
	if (!empty($x_tree_id))
		{
		$searchwhere .= "product_type.tree_id LIKE '".$x_tree_id."%'";
		}
	$_SESSION[$which_system.$modul_select . "_searchwhere"] = $searchwhere;
	}
...
if ($_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 2 ||
	...
	{
	$DefaultFilter = "store.topic_id=".$x_store_id." AND product_type.tree_id LIKE '%'";
	}
else
	{
	$DefaultFilter = viewModul($modul_select,"name").".active=1 AND store.topic_id=".$x_store_id." AND product_type.tree_id LIKE '%'";
	}
if (!$x_topic_active)
	{
	$DefaultFilter .= " AND product.id=".viewModul($modul_select,"name").".product_id ";
	$DefaultFilter .= " AND ".viewModul($modul_select,"name").".topic_id=".$key_m;
	}
checkorder();
// build SQL
$strsql = "SELECT store.*,";
$strsql .= " product.productid, product.barcode, product.name AS productname, product.novelty, product.sale,";
$strsql .= " product_maker.name AS makername, product_type.name AS typename, product_type.tree_id AS tree_id, product_size.name AS sizename";
$strsql .= " FROM store";
if ($x_category=="basket" || !$x_topic_active)
	{
	$strsql .= ", ".viewModul($modul_select,"name")." ";
	}
$strsql .= " LEFT JOIN product ON product.id=store.product_id";
$strsql .= " LEFT JOIN product_maker ON product.maker_id=product_maker.id";
$strsql .= " LEFT JOIN product_type ON product.type_id=product_type.id";
$strsql .= " LEFT JOIN product_size  ON product.size_id=product_size.id";
buildsql();
...
if ($key_m == "")
	...
	$sqlwrk = "SELECT id, name FROM " . viewModul($modul_select,"name") . "_topic";
...
$noveltySQL = "SELECT COUNT(*) AS noveltycount FROM store";
if (!$x_topic_active)
	{
	$noveltySQL .= ", ".viewModul($modul_select,"name")." ";
	}
$noveltySQL .= " LEFT JOIN product ON product.id=store.product_id";
$noveltySQL .= " WHERE";
if (!$x_topic_active)
	{
	$noveltySQL .= " product.id=".viewModul($modul_select,"name").".product_id ";
	$noveltySQL .= " AND ".viewModul($modul_select,"name").".topic_id=".$key_m ." AND";
	}
$noveltySQL .= " store.topic_id=".$x_store_id." AND product.novelty=1";
...
$html_page .= "</a>";
$html_page .= "</td></tr>";
$saleSQL = "SELECT COUNT(*) AS salecount FROM store";
if (!$x_topic_active)
	{
	$saleSQL .= ", ".viewModul($modul_select,"name")." ";
	}
$saleSQL .= " LEFT JOIN product ON product.id=store.product_id";
$saleSQL .= " WHERE";
if (!$x_topic_active)
	{
	$saleSQL .= " product.id=".viewModul($modul_select,"name").".product_id ";
	$saleSQL .= " AND ".viewModul($modul_select,"name").".topic_id=".$key_m ." AND";
	}
$saleSQL .= " store.topic_id=".$x_store_id." AND product.sale=1";
...
$html_page .= "</a>";
$html_page .= "</td></tr>";
$minimaxSQL = "SELECT COUNT(*) AS minimaxcount FROM store";
if (!$x_topic_active)
	{
	$minimaxSQL .= ", ".viewModul($modul_select,"name")." ";
	}
$minimaxSQL .= " LEFT JOIN product ON product.id=store.product_id";
$minimaxSQL .= " WHERE";
if (!$x_topic_active)
	{
	$minimaxSQL .= " product.id=".viewModul($modul_select,"name").".product_id ";
	$minimaxSQL .= " AND ".viewModul($modul_select,"name").".topic_id=".$key_m ." AND";
	}
$minimaxSQL .= " store.topic_id=".$x_store_id." AND product.description LIKE '%minimax%'";
...
$html_page .= "</a>";
$html_page .= "</td></tr>";
$basketSQL = "SELECT COUNT(*) AS basketcount FROM ".viewModul($modul_select,"name");
$basketSQL .= " LEFT JOIN product ON product.id=".viewModul($modul_select,"name").".product_id";
...
$treecountSQL .= " FROM store";
if (!$x_topic_active)
	{
	$treecountSQL .= ", ".viewModul($modul_select,"name")." ";
	}
$treecountSQL .= " LEFT JOIN product ON product.id=store.product_id";
$treecountSQL .= " LEFT JOIN product_type ON product.type_id=product_type.id";
$treecountSQL .= " WHERE";
if (!$x_topic_active)
	{
	$treecountSQL .= " product.id=".viewModul($modul_select,"name").".product_id ";
	$treecountSQL .= " AND ".viewModul($modul_select,"name").".topic_id=".$key_m ." AND ";
	}
if ($_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 2 ||
	$_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 3)
	{
	$treecountSQL .= " store.topic_id=".$x_store_id." AND product_type.tree_id LIKE " ;
	}
else
	{
	$treecountSQL .= " store.topic_id=".$s_store_id." AND store.active=1 AND product_type.tree_id LIKE ";
	}
$Page = $x_tree_page;
$treeAdmin = false;
$treeTable = "product_type";
...
if ($key_m <> "")
	{
	$strmassql = "SELECT " . viewModul($modul_select,"name") . "_topic.*,partner.name AS partnername, store_topic.name AS storename, ".viewModul($modul_select,"name")."_type.name AS typename";
	$strmassql .= " FROM " . viewModul($modul_select,"name") . "_topic";
	$strmassql .= " LEFT JOIN ".viewModul($modul_select,"name")."_type ON ".viewModul($modul_select,"name")."_type.id=".viewModul($modul_select,"name")."_topic.type_id";
	$strmassql .= " LEFT JOIN partner ON partner.id=".viewModul($modul_select,"name")."_topic.partner_id";
	$strmassql .= " LEFT JOIN store_topic ON store_topic.id=".viewModul($modul_select,"name")."_topic.store_id";
	$strmassql .= " WHERE ";	
	$strmassql .= "(".viewModul($modul_select,"name")."_topic.id = " . $key_m  . ")";	
...
if ($key_m <> "")
	...
	if (mysqli_num_rows($rsMas) > 0)
		...
		$x_topic_count = 0;
		$sqlwrk = "SELECT SUM(amount) AS topic_sum, SUM(amount*outprice) AS topic_price FROM ".viewModul($modul_select,"name");
		...
		$x_topic_price = $rowwrk["topic_price"];
		$rswrk = mysqli_query($GLOBALS['conn'],$sqlwrk);
		...
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle(viewModul($modul_select,"name")."_topic","name")."&nbsp;</span></td>";
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle(viewModul($modul_select,"name")."_topic","type_id")."&nbsp;</span></td>";
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle(viewModul($modul_select,"name")."_topic","store_id")."&nbsp;</span></td>";
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle(viewModul($modul_select,"name")."_topic","partner_id")."&nbsp;</span></td>";
		$html_page .= "<td><span class='phpmaker'>&nbsp;".viewTitle(viewModul($modul_select,"name")."_topic","delivery_letter")."&nbsp;</span></td>";
		$html_page .= "<td width='80' align='right'><span class='phpmaker'>&nbsp;".viewTitle(viewModul($modul_select,"name")."_topic","topic_sum")."&nbsp;</span></td>";
		$html_page .= "<td width='90' align='right'><span class='phpmaker'>&nbsp;".viewTitle(viewModul($modul_select,"name")."_topic","topic_outprice")."&nbsp;</span></td>";
		...
		$html_page .= "<td align='left'><span class='header'>&nbsp;" . $x_typename . "&nbsp;</span></td>";
		$html_page .= "<td align='left'><span class='header'>&nbsp;" . $x_storename . "&nbsp;</span></td>";
		$html_page .= "<td align='left'><span class='header'>&nbsp;" . $x_partnername . "&nbsp;</span></td>";
		$html_page .= "<td align='left'><span class='header'>&nbsp;" . $x_delivery_letter . "&nbsp;</span></td>";
		$html_page .= "<td align='right'><span class='phpmaker'>&nbsp;" . $x_topic_sum . "&nbsp;</span></td>";
		$html_page .= "<td align='right'><span class='phpmaker'>&nbsp;" . $x_topic_price . "&nbsp;</span></td>";
...
if ($_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 2 ||
	...
	{
	$html_page .= "<td align='left'>";
	$html_page .= orderchange("product","barcodeOrderHead");
	$html_page .= "</td>";
	}
$html_page .= "<td align='left'>";
...
$html_page .= "</td>";
$html_page .= "<td align='left'>";
$html_page .= orderchange("product","makernameOrderHead");
$html_page .= "</td>";
if ($x_topic_active)
	{
	$html_page .= "<td align='right' width='80'>";
	$html_page .= orderchange("store","amountOrderHead");
	$html_page .= "</td>";
	$html_page .= "<td align='right' width='130'>";
	$html_page .= orderchange("store","outpriceOrderHead");
	$html_page .= "</td>";
	}
if ($_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 2 ||
	$_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 3)
...
$html_page .= "<td width='100' align='center'><span class='phpmaker'>&nbsp;".viewTitle($modul_select,"price")."&nbsp;</span></td>";
$html_page .= "<td width='120' align='right'><span class='phpmaker'>&nbsp;&nbsp;".viewTitle($modul_select,"total_price")."&nbsp;&nbsp;</span></td>";
$html_page .= "<td>&nbsp;</td>";
$html_page .= "</tr>";
...
*/
		$html_page .= "<td align='left'>";
		$html_page .= "<span class='phpmaker'>" . $x_productname . "&nbsp;</span>";
		$html_page .= "</td></tr></table></td>";
		$html_page .= "<td align='left'><span class='phpmaker'>&nbsp;&nbsp;" . $row["makername"] ."&nbsp;</span></td>";
		if ($x_topic_active)
			{
			$html_page .= "<td align='right'><span class='phpmaker'>&nbsp;&nbsp;" . $row["amount"] ."&nbsp;</span></td>";
			$html_page .= "<td align='right'><span class='phpmaker'>&nbsp;&nbsp;" . $row["outprice"] ."&nbsp;</span></td>";
			}
		$html_page .= "</td>";
		$html_page .= activeTd($x_active,$enable["yes"],$enable["no"]);
		$basketsql = "SELECT * FROM ".viewModul($modul_select,"name");
		$basketsql .= " WHERE ".viewModul($modul_select,"name").".topic_id=".$key_m." AND ".viewModul($modul_select,"name").".product_id=".$x_product_id;
		$basketrs = mysqli_query($GLOBALS['conn'],$basketsql);
		...
		if ($x_topic_active)
			...
			if ($basket_is == 0)
				...
				$html_page .= "<input type='text' name='b_outprice' size='9' value='".$x_outprice."'  style='text-align:right;'>&nbsp;";
				} 
			else 
				{
				$html_page .= "<input type='text' name='b_amount' size='3' value='" . $x_basket_amount . "' style='text-align:right;color:red;'>&nbsp;";
				$html_page .= "</td><td align='center'>";
				$html_page .= "<input type='text' name='b_outprice' size='9' value='" . $x_basket_outprice . "' style='text-align:right;color:red;'>&nbsp;";
				}
			}
		else
			{
			...
			$html_page .= "</td><td align='right'>";
			$html_page .= "<span class='phpmaker'>".$x_basket_outprice."&nbsp;</span>";
			}
		$html_page .= "</td>";
		$x_total_sum = $x_basket_amount*$x_basket_outprice;
		if ($x_total_sum==0) $x_total_sum = "";
		$html_page .= "<td align='right'><span class='phpmaker'>".$x_total_sum."&nbsp;&nbsp;</span></td>";
		$html_page .= "<td align='right'><table cellspacing='0' cellpadding='0'><tr><td>";
		if (!isset($nobasket) && !$nobasket)
			{
			$html_page .= "<a href='javascript:move(" . (urlencode($key_m)).",". (urlencode($x_product_id)). ",document.listform.b_amount[" . $which_line . "].value,document.listform.b_outprice[" . $which_line . "].value);";
...
function move(t,x,y,z)
	...
	window.location = "index.php?modul_action=add&topic_id=" + t + "&key=" + x + "&amount=" + y + "&outprice=" + z;
	}
</script>
