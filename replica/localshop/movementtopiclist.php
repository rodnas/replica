...
$html_page .= "</td>";
$html_page .= "<td width='180' align='left'>";
$html_page .= orderchange($modul_select,"typenameOrderHead");
$html_page .= "</td>";
$html_page .= "<td width='180' align='left'>";
...
if ($modul_select == "instore_topic")
	{
	$html_page .= "<td width='90' align='right'><span class='phpmaker'>&nbsp;".viewTitle($modul_select,"topic_inprice")."</span></td>";
	}
$html_page .= "<td width='90' align='right'><span class='phpmaker'>&nbsp;".viewTitle($modul_select,"topic_outprice")."</span></td>";
if ($_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 2 ||
	$_SESSION[$GLOBALS["which_system"]."status_UserLevel"] == 3)
	{
...
while (($row = @mysqli_fetch_array($rs)) && ($recCount < $stopRec))
	...
	if ($recCount >= $startRec)
		...
		$html_page .= "<span class='phpmaker'>&nbsp;".htmlspecialchars($x_typename)."&nbsp;</span>";
		$html_page .= "</td>";
		$html_page .= "<td align='left'>";
		...
		if ($modul_select == "instore_topic")
			{
			$x_topic_count = 0;
			$sqlwrk = "SELECT SUM(amount) AS topic_sum, SUM(amount*inprice) AS topic_inprice, SUM(amount*outprice) AS topic_outprice FROM ".viewModul($modul_select,"itemtable");
			...
			{
			$x_topic_count = 0;
			$sqlwrk = "SELECT SUM(amount) AS topic_sum, SUM(amount*outprice) AS topic_outprice FROM ".viewModul($modul_select,"itemtable");
			if (@$_SESSION[$which_system . "status_UserLevel"] < 3)
				{
				$sqlwrk_where = "";
				}
		...
		if (($ewCurSec & ewAllowView) == ewAllowView) 
			...
			if ((@$row["id"] != NULL))
				{
				$html_page .= "<a href='index.php?modul_select=";
				$html_page .= viewModul($modul_select,"nextmodul");
				$html_page .= "&modul_action=list&key_m=" . urlencode($x_id) . "&cmd=reset'>";
				}
			else
				{
				$html_page .= "<a href=\"" . "javascript:alert('Invalid Record! Key is null');" . "\">";
