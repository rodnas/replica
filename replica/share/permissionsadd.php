...
switch ($a)
	...
	case "A": // add
		...
		sharefrompost();
		// Duplicate controll
		$strsql = "SELECT * FROM " . $modul_select . " WHERE modul='" . $x_modul . "'";
...
function  EW_checkMyForm(EW_this)
	...
	if (EW_this.x_modul && !EW_hasValue(EW_this.x_modul, "TEXT" ))
		{
		document.addform.errortext.value = "<?php echo viewModulParam($modul_select,"modulErrMsg"); ?>";
		document.addform.fieldfocus.value = "x_modul";
                return true; 
	        }
...
switch (document.addform.fieldfocus.value)
	{
	case "x_modul" : document.addform.x_modul.focus();
		break;
	case "x_name" : document.addform.x_name.focus();
		break;
	default : document.addform.x_modul.focus();
		break;
	}
-->
</script>
