...
function listnormaltype()
	...
	$list_html_page .= "<table border='0' width='100%' cellspacing='0' cellpadding='0'><tr bgcolor='".$GLOBALS["color6"]."'>";
...
function listgallerytype()
	...
	while (($row = @mysqli_fetch_array($GLOBALS["rs"])) && ($GLOBALS["recCount"] < $GLOBALS["stopRec"]))
		...
		if ($GLOBALS["recCount"] >= $GLOBALS["startRec"])
			...
			$list_html_page .= whichpictureview($x_pictURL,viewModulParam($GLOBALS["modul_select"],"viewfull"));
